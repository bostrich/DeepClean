...
import com.syezon.clean.adapter.QQDownloadFileAdapter;
...
public class QQReceiveActivity extends AppCompatActivity {
    ...
    private List<QQCacheBean> listDownload = new ArrayList<>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_qqreceive);
    ...
    private void initData() {
        for (int i = 0; i < QQCleanActivity.listRecives.size(); i++) {
            QQCacheBean bean = QQCleanActivity.listRecives.get(i);
            listDownload.add(bean);
            scanTotalSize += bean.getSize();
        }
        adapter = new QQDownloadFileAdapter(this, listDownload, new ApkItemSelectedListener() {
            @Override
